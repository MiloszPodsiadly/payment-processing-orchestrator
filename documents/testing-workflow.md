...
- `runtime-pekko` has actor/persistence tests for the Phase 3 PaymentEntity shell using Pekko Persistence TestKit, including persisted event inspection, accepted/rejected/no-op behavior, restart recovery, Pending recovery, Unknown recovery, journal write failure, corrupt history, cross-PaymentId recovery contamination, deterministic persistence IDs, current-version `PaymentEvent` serialization round-trip coverage, and rapid conflicting command serialization.
...
This is not a claim of backward-compatible event schema evolution or production upcaster
coverage.
